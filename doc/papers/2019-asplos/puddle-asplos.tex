...
\item We present PurpleDrop, a simple and affordable digital microfluidic device. Together with peripherals for heating, fluidic input/output, and volume measurement, PurpleDrop more capable than comparable designs from the literature.
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.3\linewidth}
    \centering
    \includegraphics[height=18mm]{channel-based}
  ...
  \begin{subfigure}[t]{0.3\linewidth}
    \centering
    \includegraphics[height=18mm]{robot}
...
Some channel-based devices incorporate configurable valves for some degree of flexibility \cite{amin2007aquacore}.
...
Our microfluidic programming system, Puddle, realizes this vision by making a different tradeoff than previous work in microfluidics and doing everything dynamically.
...
This section serves as an introduction to Puddle API, detailing what it provides to the programmer and the runtime system implementer.
\autoref{sec:impl} explains the implementation,
and \autoref{sec:discussion} discusses the benefits and tradeoffs of our approach.
...
Therefore, Puddle treats this case as unrecoverable and aborts.
...
Puddle only simulates droplet movement to check for errors determine the presence and location of droplets on the DMF device at each timestep.
...
We envision this make it easier to use microfluidics for complex experiment automation.
...
The systems under these abstractions cannot statically promise that that enough resources exist, but their popularity demonstrates the value of favoring expressiveness over static safety.
...
For more complex programs, it is not possible to explore all the possible sequences of fluidic operations in simulation.
Since Puddle has no knowledge of the client's program, it is entirely possible get to a state where resource requests cannot be met, forcing Puddle to crash.
We view this as a goal for future work, but also as part of the expressiveness trade-off.
...
Even though this program is practical and useful, an ahead-of-time compiler would see it as requiring unbounded\footnotemark{} resources and fail to generate a static plan.
\footnotetext{
...
When droplet \codeinline{a} is sensed, Puddle will execute the planned work from \codeinline{prep(a)}, and likewise for when \codeinline{a} is sensed.
...
One solution for this problem would be to insert \codeinline{flush(a, b)} before the calls to \codeinline{sense}.
...
\begin{acks}
  We thank the anonymous reviewers for their feedback and our shepherd Hank Hoffman for his guidance during the revision.
